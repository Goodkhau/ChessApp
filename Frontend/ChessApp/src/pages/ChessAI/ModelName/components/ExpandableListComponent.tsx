...
export default function ExpandableListComponent({ instanceKey }: { instanceKey: string }) {
	...
	return (
		<article className="h-40 w-full overflow-y-auto bg-blue-800/10 rounded-xl">
			{_.map(predictionList, prediction => (
				<article className="flex flex-row items-center m-2 px-2 bg-teal-800/20 hover:bg-teal-600/20 rounded-xl">
					...
					<div className="w-full">
						<div className="h-2 bg-green-500 rounded-xl"
							style={{ width: `${(100 * prediction.weight).toFixed(2)}%` }}
						></div>
					</div>
					<h2 className="w-1/15 ml-2">{prediction.weight.toFixed(2)}</h2>
				</article>
			))}
		</article>
	);
